...
while(1)
do
    fd_pegasus:settimeout(timeout)
    fd_stdin:settimeout(timeout)
    fd_array = { fd_stdin }
    pegasus_client = fd_pegasus:accept()
    if(pegasus_client)
    then
        fd_array[#fd_array+1] = pegasus_client
    end
    ...
    then
        ...
        _debug_mark(".")
    elseif(select_rv[1]==pegasus_client)
    then
        timeout=_ACTIVE_TIMEOUT
        print("pegasus",select_rv[1]:read())
    elseif(select_rv[1]:getfd()==0)
    then
        stdin_bytes = io.stdin:read()
        if(stdin_bytes)
        then
            timeout=_ACTIVE_TIMEOUT
