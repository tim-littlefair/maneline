...
function Fhau:check_cli_subprocess()
    ...
    then
        start_fhau_subprocess()
        start_fhau_relay_thread()
    end
...
function start_fhau_relay_thread()
    ...
    function fhau_relay_loop(fhau_flush_status, fhau_fd)
        _debug_mark("s")
        while(fhau_flush_status)
        do
           fhau_relay_cycle(fhau_flush_status, fhau_fd)
        end
        _debug_mark("x")
    end
